 ...
 public:
  explicit VulkanCtx(GlfwCtx&, std::uint32_t const version,
    ...
    auto const app_info = vk::ApplicationInfo{
        app_name, version, "No Engine", version, VK_MAKE_VERSION(1, 0, 39)};
    ...
    vk::createInstance(&inst_info, nullptr, &inst);
  ...
  int main() {
    ...
    auto const version = uint32_t{VK_MAKE_VERSION(0, 0, 1)};

    auto const app_info = vk::ApplicationInfo{
        "RANE", version, "No Engine", version, VK_MAKE_VERSION(1, 0, 39)};
    ...
    if (vk::createInstance(&inst_info, nullptr, &inst) !=
        vk::Result::eSuccess) {
      throw std::runtime_error("Failed to create Vulkan instance!");
    }

    auto vulkan_ctx = VulkanCtx{};
